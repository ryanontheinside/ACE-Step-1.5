#!/usr/bin/env python3
"""
Spike: Per-frame ODE noise injection.

Standard ODE euler loop, but after each integration step, inject small
noise per-frame scaled by a curve. Frames with high injection stay
exploratory (more creative/transformed), frames with low injection
converge normally (more faithful to source).

This is lighter than full SDE - just a small perturbation, not full
re-noising. The noise amount is scaled by both the curve and the
current sigma (so injection naturally decreases as we approach clean).

Ramp curve: 0.0 at start (no injection, faithful) -> 0.5 at end
(moderate injection, creative).
"""
...
def main():
    print("=" * 70)
    print("SPIKE: Per-frame ODE noise injection")
    ...
    # ================================================================
    # 5. Build noise injection curve
    # ================================================================
    print("\n5. Build noise injection curve (1Hz sine)...")
    import math
    fps = T / 60.0
    t_sec = torch.arange(T, device=device, dtype=dtype) / fps
    # 1Hz sine mapped to [0, 0.5]
    inject_curve = 0.25 * (1.0 + torch.sin(2.0 * math.pi * 1.0 * t_sec))
    inject_3d = inject_curve.unsqueeze(0).unsqueeze(-1)  # [1, T, 1]
    print(f"   injection: 1Hz sine, range 0.0 -> 0.5")

    # ================================================================
    # 6. Diffusion loop with per-frame noise injection
    # ================================================================
    print("\n6. Diffusion loop (ODE + per-frame noise injection)...")
    ...
    with handler._load_model_context("model"):
        for i in range(steps):
            ...
            # Standard ODE euler step
            if i == steps - 1:
                xt = xt - tc * vt
            else:
                xt = xt - vt * (tc - tn)

                # Per-frame noise injection (skip on final step)
                injection_noise = torch.randn_like(xt)
                xt = xt + injection_noise * inject_3d
    ...
    path = os.path.join(OUTPUT_DIR, "ode_noise_injection_ramp.wav")
